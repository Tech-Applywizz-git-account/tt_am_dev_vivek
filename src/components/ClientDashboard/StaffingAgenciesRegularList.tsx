...
import { Calendar, Building2, MapPin, ExternalLink, ChevronDown, ChevronUp, Loader2, Briefcase, DollarSign, Building, Monitor } from "lucide-react";
...
interface SummaryResponse {
    staffing_jobs: Record<string, number>;
    summary: {
        total: number;
...
interface StaffingAgenciesRegularListProps {
    applywizzId?: string;
}
...
const StaffingAgenciesRegularList: React.FC<StaffingAgenciesRegularListProps> = ({ applywizzId }) => {
    ...
    const fetchJobsForDate = async (date: string) => {
        ...
        try {
            ...
            // API returns staffing_jobs array for industry_type=true
            setJobsData(prev => ({
                ...prev,
                [date]: data.staffing_jobs || data.jobs || []
            }));
        } catch (err) {
    ...
    const renderJobCard = (job: JobItem, date: string) => {
        ...
        const fallbackAvatarUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(job.company || 'Company')}&background=f97316&color=fff&size=128&bold=true&rounded=true`;
            ...
            <div key={job.id} className="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden border border-gray-100">
                ...
                <div className="px-6 pb-6 flex items-center gap-3">
                    <select
                        ...
                        className="px-4 py-2.5 border border-gray-300 rounded-lg text-sm font-medium focus:outline-none focus:ring-2 focus:ring-orange-500 bg-white hover:bg-gray-50 transition-colors"
                    >
                    ...
                    <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="px-6 py-2.5 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-bold rounded-lg transition-all duration-200 shadow-md hover:shadow-lg flex items-center gap-2"
                    >
        ...
        return (
            <div className="bg-white p-4 rounded-lg shadow mt-6">
                <div className="flex items-center gap-2 mb-6">
                    ...
                    <span className="text-gray-700 font-medium">Loading Staffing Agency jobs...</span>
                </div>
    ...
    if (dates.length === 0) {
        return (
            <div className="bg-white p-4 rounded-lg shadow mt-6">
                <p className="text-gray-500">No staffing agency jobs found.</p>
            </div>
        );
    }
                    ...
                    return (
                        <div key={date}>
                            <div
                                ...
                                className={`flex justify-between items-center p-4 rounded-lg cursor-pointer transition ${isExpanded ? "bg-orange-100" : "bg-orange-50 hover:bg-orange-100"
                                    }`}
                            >
...
export default StaffingAgenciesRegularList;
