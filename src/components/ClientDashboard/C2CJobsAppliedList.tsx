...
import { CheckCircle, MapPin, ExternalLink, Loader2, ChevronLeft, ChevronRight, DollarSign } from "lucide-react";
...
interface C2CJobsAppliedListProps {
    applywizzId?: string;
}

const C2CJobsAppliedList: React.FC<C2CJobsAppliedListProps> = ({ applywizzId }) => {
    ...
    const loadingMessages = [
        "Fetching c2c agency applications...",
        "Scanning c2c jobs...",
        "You're almost there, thanks for the patience!",
        "Preparing your c2c jobs dashboard...",
    ];
    ...
    const fetchAppliedJobs = async (page: number = 1) => {
        ...
        try {
            ...
            const url = `${apiUrl}/api/job-links?lead_id=${applywizzId}&page=${page}&page_size=${pageSize}&status=Completed&job_type=C2C`;
    ...
    const getMatchQuality = (score: number) => {
        ...
        if (percentage >= 70) return { label: 'GOOD MATCH', bgColor: 'bg-gradient-to-br from-purple-900 to-purple-800', textColor: 'text-purple-400' };
        ...
        return (
            <div className="flex items-center justify-between mt-6 px-4">
                <div className="text-sm text-gray-600">
                    Showing {jobs.length} of {totalJobs} applied c2c jobs
                </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md mt-6 animate-pulse">
                <div className="flex flex-col items-center justify-center py-12">
                    ...
                    <p className="text-sm text-gray-500 mt-2">
                        We are currently gathering your c2c agency records...
                    </p>
                </div>
            </div>
        );
    ...
    if (jobs.length === 0) {
        return (
            <div className="bg-white p-4 rounded-lg shadow mt-6">
                <p className="text-gray-500">No applied c2c agency jobs found.</p>
            </div>
        );
    }
        ...
        <div className="bg-white p-4 rounded-lg shadow mt-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <h2 className="text-lg font-semibold flex items-center gap-2">
                    ...
                    Applied c2c Agency Jobs ({totalJobs})
                </h2>
            </div>
...
export default C2CJobsAppliedList;
